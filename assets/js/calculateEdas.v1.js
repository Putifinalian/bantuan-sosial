class calculateEDAS {
    constructor() {
        ...
        if (document.querySelector(this.$table_selector)) {
            ...
            if (use_log) console.log("this.$id_kriteria", this.$id_kriteria);
            if (use_log) console.log("this.$kriteria", this.$kriteria);
    ...
    matrik_keputusan() {
        const _this = this
        let el_table = document.querySelector(this.$table_selector)
        let results = []
        el_table.querySelectorAll("tbody tr").forEach((tr_el, tr_i) => {
            let data = {
                id_calon_penerima: tr_el.querySelector(`[data-id_calon_penerima]`).getAttribute("data-id_calon_penerima"),
            }
            tr_el.querySelectorAll(`[data-id_kriteria]`).forEach((el, i) => {
                data[el.getAttribute("data-id_kriteria")] = Number(el.innerText)
            })

            results.push(data)
        })

        el_table.querySelectorAll("thead th[data-id_kriteria]").forEach((th_el, th_i) => {
            let id_kriteria = th_el.getAttribute("data-id_kriteria")
            let tipe = th_el.getAttribute("data-tipe")
            let bobot = th_el.getAttribute("data-bobot")
            if (id_kriteria) _this.$id_kriteria.push(id_kriteria)

            if (id_kriteria && !_this.$kriteria[id_kriteria]) {
                _this.$kriteria[id_kriteria] = {}
                _this.$kriteria[id_kriteria]["bobot"] = Number(bobot)
                _this.$kriteria[id_kriteria]["tipe"] = tipe
            }
        })
